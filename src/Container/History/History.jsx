...
const History = () => {
  ...
  useEffect(() => {
    const fetchUserDataAndHistory = async () => {
      try {
        ...
        const historyDocRef = doc(db, "history", data.uid);
        const historySongsDocSnapshot = await getDoc(historyDocRef);

        if (historySongsDocSnapshot.exists()) {
          const historySongsData = historySongsDocSnapshot.data().history || [];
          setHistorySongs(historySongsData);
          localStorage.setItem("history", JSON.stringify(historySongsData));
        } else {
 ...
 const handleNext = () => {
   if (index < historySongs.length - 1) {
     setIndex(index + 1);
     playSong(
       historySongs[index + 1]?.file,
       historySongs[index + 1]?.fileName,
       historySongs[index + 1]?.image,
       index + 1
     );
     setMusic(0);
   } else {
     ...
     playSong(
       historySongs[0]?.file,
       historySongs[0]?.fileName,
       historySongs[0]?.image,
       0
     );
     setMusic(0);
   }
 ...
 const handlePrev = () => {
   if (index > 0) {
     ...
     setIndex(historySongs.length - 1);
     playSong(
       historySongs[historySongs.length - 1]?.file,
       historySongs[historySongs.length - 1]?.fileName,
       historySongs[historySongs.length - 1]?.image,
    ...
    useEffect(() => {
      if (audioPlayer) {
        ...
        const handleEnded = () => {
          ...
          console.log(historySongs[index +1]?.file);
          setAudioPlayer(
            new Audio(historySongs[index + 1]?.file)
          );
          setIndex(index + 1);
          setMusic(0);
          setIsPlaying(false);
            ...
            <div className="heroSection w-[70vw] md:w-[80vw] h-[70vh] overflow-scroll">
              <div className="md:w-[75vw] w-[65vw] mx-auto h-full">
                <div className="grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto ">
                  {historySongs.map((song, songIndex) => (
                    <div
                      onClick={() => {
                        playSong(
                          historySongs[songIndex]?.file,
                          historySongs[songIndex]?.fileName,
                          historySongs[songIndex]?.image,
                          songIndex
                        );
                      }}
                      key={song.file}
